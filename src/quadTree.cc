...
QuadTreeNode *QuadTree::createRootNode(std::vector<Body> &bodies) {
  ...
  // squarify bounds:
  double maxSide = std::max(x2 - x1, y2 - y1);
...
void QuadTree::updateBodyForce(Body *sourceBody) {
  ...
  while (queueLength) {
    ...
    if (body != NULL && differentBody) {
      ...
      // TODO: Reduced r^3 to r^2.. not 100% sure that was right
      v = layoutSettings->gravity * body->mass * sourceBody->mass / (r * r);
      fx += v * dx;
      fy += v * dy;
      ...
      if ((node->right - node->left) / r < layoutSettings->theta) {
        ...
        // TODO: Reduced r^3 to r^2.. not 100% sure that was right
        v = layoutSettings->gravity * node->mass * sourceBody->mass / (r * r);
        fx += v * dx;
        fy += v * dy;
      } else {
